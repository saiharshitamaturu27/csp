...
export function StatCard({ icon: Icon, label, value, color = 'primary', trend }: {
  ...
  const accentMap: Record<string, string> = {
    primary: '#10b981', secondary: '#3b82f6', accent: '#f97316',
    success: '#22c55e', warning: '#f59e0b', error: '#ef4444',
  };
  return (
    <div className="stat-card card-hover" style={{ '--stat-accent': accentMap[color] } as React.CSSProperties}>
...
export function Modal({ open, onClose, title, children, size = 'md' }: {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start sm:items-center justify-center p-4 overflow-y-auto">
      ...
      <div className={`relative bg-white rounded-2xl shadow-2xl w-full ${sizeMap[size]} my-8 animate-scale-in`}>
